...
async def composed(run_id: str, request: Request):
    ...
    accepted_ids = {comp.get("id") for comp in result.accepted}
    root_id = surf.get("root")
    if root_id not in accepted_ids and result.accepted:
        layout = next(
            (comp for comp in result.accepted
             if comp.get("type") in {"Column", "Row", "List", "Card", "Tabs"}),
            result.accepted[0],
        )
        root_id = layout.get("id")
    return {
        "run_id": run_id,
        "finished": run["finished"],
        "surface": {"root": root_id, "components": result.accepted,
...
@router.get("/app/trip", response_class=HTMLResponse)
@router.get("/app/trip/", response_class=HTMLResponse)
async def trip_viewer():
    """Trip Planner: the agent answers only by composing interfaces across turns."""
    path = Path(__file__).parent / "client" / "trip.html"
    if not path.exists():
        raise HTTPException(500, "trip viewer missing")
    return path.read_text(encoding="utf-8")

@router.get("/app/fitness", response_class=HTMLResponse)
@router.get("/app/fitness/", response_class=HTMLResponse)
async def fitness_viewer():
    """Fitness Coach: the agent orchestrates a 3-turn interactive workout flow."""
    path = Path(__file__).parent / "client" / "fitness.html"
    if not path.exists():
        raise HTTPException(500, "fitness viewer missing")
    return path.read_text(encoding="utf-8")
